...
//This mapper is used to select place_name and tags out from combining the output file of PlaceFilterMapper with photo.txt.
...
//3. output place_name and tags
//Input format: place_id \t place_name
//Output format: place_name \t tags
...
public class ReplicateJoinMapper extends Mapper<Object, Text, Text, Text>
{
	...
	private Text placeName = new Text(), photoTags = new Text();
	...
	public void map(Object key, Text value, Context context) throws IOException, InterruptedException 
	{
		...
		{
			System.out.println(placeTable.get(place_Id).trim());
			placeName.set(placeTable.get(place_Id).trim());
			
			//Get tags
			String tags = dataArray[2].trim();
			System.out.println(tags);
			photoTags.set(tags);
			context.write(placeName, photoTags);
		}
		else
		{
			return;
